...
class Parser[T]:
    def __init__(self, parse: Callable[[str], Result[ParseSuccess[T], ParseError]]):
        self.parse = parse
    ...
    def between(self, d1: 'Parser[Any]', d2: 'Parser[Any]') -> 'Parser[T]':
        ...
        return d1.ignore_then(self).then_ignore(d2)
    ...
    @staticmethod
    def char(f: Callable[[str], bool]) -> 'Parser[str]':
    ...
    def just(token: str) -> 'Parser[str]':
        ...
        def new_parser(input: str) -> Result[ParseSuccess[str], ParseError]:
            ...
            return Err(f"Expected '{token}', found '{input[len(token):]}'")
        return Parser(new_parser)
